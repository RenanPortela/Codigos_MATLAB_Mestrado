function [KG] = stiffness_matrix_HSDT(E, poisson, thickness, mesh)

% GLOBAL STIFFNESS MATRIX PRE-ALOCATION
KG = zeros(7*mesh.nnodes);

A = thickness*E/(1-poisson^2)*[1 poisson 0;poisson 1 0;0 0 (1-poisson)/2];
D = thickness^3/12*E/(1-poisson^2)*[1 poisson 0;poisson 1 0;0 0 (1-poisson)/2];
F = thickness^5/80*E/(1-poisson^2)*[1 poisson 0;poisson 1 0;0 0 (1-poisson)/2];
H = thickness^7/448*E/(1-poisson^2)*[1 poisson 0;poisson 1 0;0 0 (1-poisson)/2];

G = thickness*E/2/(1+poisson)*eye(2);
S = thickness^3/12*E/2/(1+poisson)*eye(2);
T = thickness^5/80*E/2/(1+poisson)*eye(2);
...
switch mesh.type
    ...
    case 'Q9'
        [gaussWeights,gaussLocations] = gaussQuadrature_Q9('reduced');
end
